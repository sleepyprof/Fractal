...
public abstract class FractalZoomSimpleIterateModelAdapter<D> extends FractalZoomIterateModelAdapter<D> {

	private final FractalZoomWritableBuffer<D> buffer;
            ...
            for (int frame = 0; frame < getFrames(); frame++) {
                ...
                for (int y = 0; y < getHeight(); y++) {
                    for (int x = 0; x < getWidth(); x++) {
                        ...
                        Coordinate c = getCoordinate(x, y, frame);
	...
	public FractalZoomSimpleIterateModelAdapter(FractalZoomWritableBuffer<D> buffer, FractalIteratorFactory<Coordinate> iteratorFactory, int maxiter, boolean julia) {
		super(buffer, iteratorFactory, maxiter);
		this.buffer = buffer;
		this.julia = julia;
	}

    public FractalZoomSimpleIterateModelAdapter(FractalZoomWritableBuffer<D> buffer, FractalIteratorFactory<Coordinate> iteratorFactory, int maxiter) {
